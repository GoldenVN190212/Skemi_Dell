// ========================================
// AI NOTEBOOK CONTROLLER - NotebookLM Style
// ========================================
...
class AINotebookController {
    ...
    bindEvents() {
        ...
        // Toolbar events
        document.getElementById('autoGenerateBtn')?.addEventListener('click', () => this.toggleMode('auto'));
        document.getElementById('manualDrawBtn')?.addEventListener('click', () => this.toggleMode('manual'));
    ...
    async analyzeFile(fileData) {
        ...
        try {
            ...
            this.addMessage('ai', 'Dịch vụ hiện tạm gián đoạn. Vui lòng kiểm tra kết nối mạng và thử lại.');
        }
    }
    ...
    renderMermaidDiagram(diagramData) {
        const container = document.getElementById('diagramContainer');
        container.innerHTML = `<div class="mermaid">${diagramData.mermaid}</div>`;
    ...
    renderTimeline(timelineData) {
        const container = document.getElementById('diagramContainer');
        // Simple timeline HTML rendering
        container.innerHTML = `
    ...
    initManualDrawing() {
        // Simple canvas drawing implementation
        const container = document.getElementById('diagramContainer');
    ...
    async processAIMessage(message) {
        try {
            const response = await fetch('/notebook_chat', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    message: message,
