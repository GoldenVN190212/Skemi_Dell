...
class AIChatWidget {
    constructor() {
        ...
        this.chatStorage = this.resolveSessionStorage();
        this.chatHistoryKey = 'skemi_ai_chat_history_v2';
        this.sessionIdKey = 'skemi_ai_session_id_v1';
        ...
        this.pendingCommandKey = 'skemi_ai_pending_command_v1';
        this.workflowStateKey = 'skemi_ai_workflow_state_v1';
        this.projectStorageKey = 'skemi_studio_projects_v1';
        this.searchStateKey = 'skemi_search_state_v3';
        this.quizRoomStorageKey = 'skemi_private_quiz_rooms_v1';
    ...
    registerEphemeralLifecycle() {
        const cleanup = () => this.clearEphemeralSession({ notifyBackend: true });
    ...
    async requestAssistantStream(payload, options = {}) {
        const candidates = [this.apiBaseUrl, 'http://127.0.0.1:8010']
    ...
    async sendMessage() {
        ...
        try {
            ...
            if (command) {
                ...
                const localResult = await this.runApprovedCommand(command);
                // Kết quả (answer và navigateTo) hiện đã được xử lý ngay bên trong runApprovedCommand cho từng bước
                return;
            }

            streamView = this.createStreamingAssistantMessage();
            const data = await this.requestAssistantStream({
                session_id: this.sessionId,
                question: message,
                age_group: this.ageGroup
            }, {
                signal: this.requestAbortController.signal,
                onEvent: (payload) => this.handleAssistantStreamEvent(streamView, payload)
            });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const isChatPage = /\/(skemma\/)?chat\.html$/i.test(pathname);
    if (isChatPage) {
        return;
    }
    ...
    if (true) {
        ...
        const shouldAutoOpen = new URLSearchParams(window.location.search).get('openChat') === '1';
        if (shouldAutoOpen) {
